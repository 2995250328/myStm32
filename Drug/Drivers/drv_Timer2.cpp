...
extern int State_Back;
extern u8 Blooth_Uart_Tx_Buffer[8];
extern "C" void TIM2_IRQHandler(void)
{ 		    		  			    
	if(TIM_GetITStatus(TIM2, TIM_IT_Update) == SET)
	{    
		TIM_ClearITPendingBit(TIM2, TIM_IT_Update ); //清除 TIM2 更新中断标志	
		if(Delay_Transmit<5)//每0.1s发送一次数据包给k210
			Delay_Transmit++;
		else 
			Delay_Transmit=0;
		if(time_cnt<60)
		...
		if(State_Back==5)//返回走长直道的状态
		{
			if((Location_CM_L+Location_CM_R)/2>103)//小车一走到中段十字路口前
				Blooth_Uart_Tx_Buffer[5]=1;//此位用来传输临时停车结束的数据
		}
		...
		if(Spin_Flag==1)//转向控制
		{
			if(Position_L<(Target_Position_L+30)&&Position_L>(Target_Position_L-30))
			{
				Spin_Succeed[0]=1;
				LV_PWM_L=Location_Speed_L(Encoder_Left,Position_L,Target_Position_L,0);
			}
			else
			{
				Spin_Succeed[0]=0;
				if(Little_Flag==0)
					LV_PWM_L=Location_Speed_L(Encoder_Left,Position_L,Target_Position_L,Speed_Max*0.57);
				else
					LV_PWM_L=Location_Speed_L(Encoder_Left,Position_L,Target_Position_L,Speed_Max*0.26);
			}
			if(Position_R<(Target_Position_R+30)&&Position_R>(Target_Position_R-30))
			{
				Spin_Succeed[1]=1;
				LV_PWM_R=Location_Speed_R(Encoder_Right,Position_R,Target_Position_R,0);
			}
			...
			{	
				Spin_Succeed[1]=0;
				if(Little_Flag==0)
					LV_PWM_R=Location_Speed_R(Encoder_Right,Position_R,Target_Position_R,Speed_Max*0.57);
				else
					LV_PWM_R=Location_Speed_R(Encoder_Right,Position_R,Target_Position_R,Speed_Max*0.26);
			}
