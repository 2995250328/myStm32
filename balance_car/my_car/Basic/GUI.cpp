...
void LCD_ShowMAIN()
{
  ...
  LCD_ShowString(0,48,YAW,BLACK,WHITE,16,0);
	...
	if(Encoder_Left<0)
	{
		LCD_ShowChar(104,64,'-',PURPLE,WHITE,16,0);
		Encoder_Left_temp=0-Encoder_Left;
	}
	else
	{
		LCD_ShowChar(104,64,' ',WHITE,WHITE,16,0);
		Encoder_Left_temp=Encoder_Left;
	}
	if(Encoder_Right<0)
	{
		LCD_ShowChar(112,80,'-',PURPLE,WHITE,16,0);
		Encoder_Right_temp=0-Encoder_Right;
	}
	else
	{
		LCD_ShowChar(112,80,' ',WHITE,WHITE,16,0);
		Encoder_Right_temp=Encoder_Right;
	}
	LCD_ShowIntNum(112,64,Encoder_Left_temp,6,PURPLE,WHITE,16);
	LCD_ShowIntNum(120,80,Encoder_Right_temp,6,PURPLE,WHITE,16);
	//如果电压过低 提示充电
	if(Volt_flag==1)
		LCD_ShowString(32,100,VOLT,RED,WHITE,24,0);
}
