...
{ 		    		  			    
	...
	{    
		...
		{
			if((Location_CM_L+Location_CM_R)/2>98)//小车一走到中段十字路口前
		...
		if(Spin_Flag==1)//转向控制
		{
			...
			else
			{
				Spin_Succeed[0]=0;
				if(Little_Flag==0)
					LV_PWM_L=Location_Speed_L(Encoder_Left,Position_L,Target_Position_L,Speed_Max*0.41);
				else
					LV_PWM_L=Location_Speed_L(Encoder_Left,Position_L,Target_Position_L,Speed_Max*0.20);
			}
			...
			{	
				Spin_Succeed[1]=0;
				if(Little_Flag==0)
					LV_PWM_R=Location_Speed_R(Encoder_Right,Position_R,Target_Position_R,Speed_Max*0.41);
				else
					LV_PWM_R=Location_Speed_R(Encoder_Right,Position_R,Target_Position_R,Speed_Max*0.20);
			}
