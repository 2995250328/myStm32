...
//位置PID系数
float Position_KP_L=75.79,Position_KI_L=0.37,Position_KD_L=185; 	
//速度PID系数
float Velocity_KP_L=80,Velocity_KI_L=9.51,Velocity_KD_L=20; 
//位置PID系数
float Position_KP_R=89.5,Position_KI_R=0.57,Position_KD_R=185; 
//速度PID系数 
float Velocity_KP_R=10,Velocity_KI_R=9.5,Velocity_KD_R=25.6; 
//寻迹PID系数	
float OpenMv_KP=22,OpenMv_KI=0.43,OpenMv_KD=30;
//位置PID设定目标数
...
{	
	...
	{	
		...
		{		
			LCD_ShowMAIN();		
			Blooth_Uart_Tx_Buffer[3]=Target_Room;//一直发送目标房间
			if(Init_Flag<50)
			{
			...
			else if(Target_Room=='C')
			{
				switch(State_Back)
				{
					...
					case 1:
						if(Stop_Flag==1)
						{
							Blooth_Uart_Tx_Buffer[2]=0x01;//小车2出发
							Back_Flag=0;
							Car_Spin(left_90);
							State_Back++;
						}
