...
int is_mpu6050_still(float *acc,float *gyro)
{
    // 定义加速度值和角速度值
    float ax, ay, az, gx, gy, gz;
		ax=acc[0]/16384;ay=acc[1]/16384;az=acc[2]/16384;
		gx=gyro[0];gy=gyro[1];gz=gyro[2];

    // 计算合成角速度
    float motion_gyro=sqrt(gx * gx + gy * gy + gz * gz);
    // 计算总加速度
    float motion_accel = sqrt(ax * ax + ay * ay + az * az);

    // 判断加速度是否小于阈值
    if (motion_accel < 1.6 && motion_gyro < 1.25 && motion_accel < 0.65) {
        return 1;
    } else {
        return 0;
    }
