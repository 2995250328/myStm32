...
extern int key_flag,key_debug,key_turn,function_flag;
...
void LCD_ShowMAIN()
{
	int Encoder_Left_temp,Encoder_Right_temp,mtl_temp,mtr_temp;
	...
	if(function_flag==1)
	{
		...
		LCD_ShowString(0,128,MTL,BLACK,WHITE,16,0);
		LCD_ShowString(0,144,MTR,BLACK,WHITE,16,0);
		//显示电压
		...
		LCD_ShowFloatNum1(32,96,Velocity_Left_temp,6,PURPLE,WHITE,16);
		LCD_ShowFloatNum1(32,112,Velocity_Right_temp,6,PURPLE,WHITE,16);
		/*获取PWM赋值的速度*/
		if(Motor_Left<0)
		{
			LCD_ShowChar(32,128,'-',PURPLE,WHITE,16,0);
			mtl_temp=0-Motor_Left;
		}
		else
		{
			LCD_ShowChar(32,128,' ',WHITE,WHITE,16,0);
			mtl_temp=Motor_Left;
		}
		if(Motor_Right<0)
		{
			LCD_ShowChar(32,144,'-',PURPLE,WHITE,16,0);
			mtr_temp=0-Motor_Right;
		}
		else
		{
			LCD_ShowChar(32,144,' ',WHITE,WHITE,16,0);
			mtr_temp=Motor_Right;
		}
		LCD_ShowIntNum(40,128,mtl_temp,4,PURPLE,WHITE,16);
		LCD_ShowIntNum(40,144,mtr_temp,4,PURPLE,WHITE,16);
	}
	//功能选择显示
	if(key_flag==0)
	{
		if(function_flag==1)
			LCD_ShowString(0,160,BLU,BLUE,WHITE,16,0);
		else
			LCD_ShowString(0,160,BLU,RED,WHITE,16,0);
	}
	else if(key_flag==2)
	{
		if(key_turn==0)
		{
			if(function_flag==0)
				LCD_ShowString(0,160,TURN1,RED,WHITE,16,0);
			else
				LCD_ShowString(0,160,TURN1,BLUE,WHITE,16,0);
		}
		else if(key_turn==1)
		{
			if(function_flag==0)
				LCD_ShowString(0,160,TURN2,RED,WHITE,16,0);
			else
				LCD_ShowString(0,160,TURN2,BLUE,WHITE,16,0);
		}
		else if(key_turn==2)
		{
			if(function_flag==0)
				LCD_ShowString(0,160,TURN3,RED,WHITE,16,0);
			else
				LCD_ShowString(0,160,TURN3,BLUE,WHITE,16,0);
		}
	}
	
	LCD_ShowIntNum(0,200,Uart1_Receive,3,RED,WHITE,24);
}
/******************************************************************************
函数说明：Debug模式下的显示
      入口数据：无
...
void LCD_ShowDebug()
{
	...
	u8 DEBUG2[]={"Debug Vel        "};
		...
		{
			if(function_flag==0)
				LCD_ShowString(0,160,DEBUG1,RED,WHITE,16,0);
			else
				LCD_ShowString(0,160,DEBUG1,BLUE,WHITE,16,0);
		}
		else if(key_debug==1)
		{
			if(function_flag==0)
				LCD_ShowString(0,160,DEBUG2,RED,WHITE,16,0);
			else
				LCD_ShowString(0,160,DEBUG2,BLUE,WHITE,16,0);
		}
}
