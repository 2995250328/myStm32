...
{ 		    		  			    
	...
	{    
		...
		{
			if(offset_flag==1&&Volt_flag==0&&Debug==0&&function_flag==1)
			{
				...
				else 
				{
					...
					Motor_Left=Balance_Pwm+Velocity_Pwm+Turn_Pwm+Position_Moto_L;       //计算左轮电机最终PWM
					Motor_Right=Balance_Pwm+Velocity_Pwm-Turn_Pwm+Position_Moto_R;      //计算右轮电机最终PWM
		...
		if(offset_flag==1&&function_flag==1&&Debug==1)
		{
				if (Debug_pos)
				{
					Motor_Left=Position_PID_L(Encoder_Left,Target_Position_L);
	...
	//    		Motor_Right=PWM_Limit(Motor_Right,6700,-6700);
				...
				{
					Motor_Left=Incremental_PI_L(Encoder_Left,Target_Velocity_L);
					Motor_Left=PWM_Limit(Motor_Left,6700,-6700);
	//				Motor_Right=Incremental_PI(Encoder_Right,Target_Velocity_R);
	//				Motor_Right=PWM_Limit(Motor_Right,6700,-6700);
					Set_Pwm(Motor_Left,0);
				}
		}
