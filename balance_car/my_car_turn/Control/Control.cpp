...
{
	static float turn,angle_bias,gyro_bias;
	angle_bias=Turn_Target-yaw;
	
	if(angle_bias<0&&fabs(angle_bias)>7&&key_flag!=3&&key_flag!=0)
		angle_bias+=360;
	if(key_flag==3)
	{
		if(flag_360==0&&angle_bias<0)
			angle_bias+=360;
		else if(angle_bias<7&&angle_bias>0&&flag_360!=1)
			flag_360=1;	
		if(angle_bias<-180&&flag_360==1)
			angle_bias=angle_bias+360;
		else if(angle_bias>180&&flag_360==1)
			angle_bias=angle_bias-360;
	}
	if(key_flag==0)
	{
	...
	gyro_bias=0-gyro;
	turn=-angle_bias*Turn_Kp-gyro_bias*Turn_Kd;//结合Z轴陀螺仪进行PD控制
	return turn;									 				
}