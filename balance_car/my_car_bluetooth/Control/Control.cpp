...
int Velocity(int encoder_left,int encoder_right)
{
	...
	//前进后退控制
	if(Flag_front==1)    	Movement=20;	  //收到前进信号
	else if(Flag_back==1)	Movement=-20;  //收到后退信号
	else  Movement=0;	
	//速度 PI 控制器
	Encoder_Least =0-(encoder_left+encoder_right); //获取最新速度偏差=目标速度（此处为零）-测量速度（左右编码器之和）
	Encoder_bias *= 0.75; //一阶低通滤波器
	Encoder_bias+= Encoder_Least*0.25; //一阶低通滤波器
...
int Turn(float yaw,float gyro)
{
	...
	float Kd;
		...
		if(1==Flag_front||1==Flag_back)  
			...
			Kd=0;   //转向的时候取消陀螺仪的纠正 
	turn=-Turn_Target*Turn_Kp-gyro*Kd;//结合Z轴陀螺仪进行PD控制
