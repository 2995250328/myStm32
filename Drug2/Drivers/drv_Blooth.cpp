...
Jump:
    ...
    Blooth_Uart_Rx_Index = 0;
		if(Blooth_Uart_Rx_Buffer[3]=='C'||Blooth_Uart_Rx_Buffer[3]=='D')
		{
			Target_Room=Blooth_Uart_Rx_Buffer[3];
			Mission=1;//1车走到中段实行任务1
		}
		else if(Blooth_Uart_Rx_Buffer[3]=='E'||Blooth_Uart_Rx_Buffer[3]=='F'||Blooth_Uart_Rx_Buffer[3]=='G'||Blooth_Uart_Rx_Buffer[3]=='H')
			Mission=2;//1车走到远端实行任务2
		if(Blooth_Uart_Rx_Buffer[2]==1)
			Car2=1;
//				for(int i=0;i<8;i++)
